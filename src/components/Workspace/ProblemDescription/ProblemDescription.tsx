		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (userDoc.exists() && problemDoc.exists()) {
				if (liked) {
					...
					setCurrentProblem((prev) => (prev ? { ...prev, likes: Number(prev.likes - 1) || 0 } : null));
					...
					setCurrentProblem((prev) =>
						prev ? { ...prev, likes: prev.likes + 1, dislikes: Number(prev.dislikes - 1) || 0 } : null
					);
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (userDoc.exists() && problemDoc.exists()) {
				...
				if (disliked) {
					...
					setCurrentProblem((prev) => (prev ? { ...prev, dislikes: Number(prev.dislikes - 1) || 0 } : null));
					...
					setCurrentProblem((prev) =>
						prev ? { ...prev, dislikes: prev.dislikes + 1, likes: Number(prev.likes - 1) || 0 } : null
					);
